...
def doXnStreamUncompressDepth16ZWithEmbTablePTR(input,outputptr,outputlen):
	r = ctypes.c_int(outputlen);
	rr = XnStreamUncompressDepth16ZWithEmbTable(ctypes.c_char_p(input),len(input),outputptr,ctypes.byref(r))
	return (rr,r.value)
...
def doXnStreamCompressDepth16ZWithEmbTablePTR(inputptr,inputlen,outputptr,outputlen,maxvalue):
	r = ctypes.c_int(outputlen);
	rr = XnStreamCompressDepth16ZWithEmbTable(inputptr,inputlen,outputptr,ctypes.byref(r),maxvalue)
	return (rr,r.value)
