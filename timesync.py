#https://github.com/ros/ros_comm/blob/247459207e20c1da109fc306e58b84d15c4107bd/utilities/message_filters/src/message_filters/__init__.py
...
class TimeSynchronizer(SimpleFilter):
    ...
    def add(self, msg, my_queue, my_queue_index=None):
        ...
        common = reduce(set.intersection, [set(q.keys()) for q in self.queues])
        if False:
            if my_queue_index == 0:
                print "---"
                for i,q in enumerate(self.queues):
                    print i,sorted(q.keys())
    ...
    def add(self, msg, my_queue, my_queue_index=None):
        ...
        for vv in itertools.product(*[zip(*s)[0] for s in stamps]):
            ...
            if ( ((max(vv) - min(vv)) < self.slop) and
                ...
                tt = [t for q,t in qt]
                print "notify",[t-tt[0] for t in tt]
                self.signalMessage(*msgs)
                for q,t in qt:
                    del q[t]
